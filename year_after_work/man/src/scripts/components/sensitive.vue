    ...
    <div class="modal fade bs-example-modal-lg" id="addAgentModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                    <h4 class="modal-title" id="myModalLabel" style="text-align: center">新增敏感词</h4>
                </div>
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
                        <div class="form-group">
                            <label for="inputEmail1" class="col-sm-2 control-label">content (*)</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="content" v-model.trim="newcontent">
                            </div>
                        </div>
                        <div class="addTips">
                            {{addATips1}}
...
export default {
    ...
    methods: {
        unionList(page, size) {
            const that = this;
            $.ajax({
                url: that.$store.state.rootUrl3 + '/sensitiveword/list',
                headers: {
        ...
        addUnion() {
            ...
            if (that.newcontent == '') {
                ...
                $.ajax({
                    url: that.$store.state.rootUrl3 + '/sensitiveword/create',
                    type: "POST",
                    headers: {
                        "token": localStorage.getItem('A-TOKEN')
                    },
        ...
        deleteList() {
            const that = this;
            $.ajax({
                url: that.$store.state.rootUrl3 + '/sensitiveword/delete',
                headers: {
                    "token": localStorage.getItem('A-TOKEN')
                },
                type: "POST",
