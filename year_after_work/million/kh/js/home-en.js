...
function getUserData(){
    ...
    $.ajax({
        ...
        data: {
            "loginid": obj.loginid
            // "cc": obj.cc
            // loginid: '000192c3655beac27694d074'
        },
        success: function(data){
            var code = data.returncode;
...
function userDataCB(data){
    ...
    if( data.liveid ){
        $(".goBtn").html("Join QH").attr("data-id", data.liveid);
        // $(".goBtn").html("开始答题").attr("data-id", data.liveid);
    }else{
        $(".goBtn").html("Coming soon").attr("data-id", '');
    ...
    $(".balance").attr("data-cash", data.cash).find(".des-num").html(toThousands(data.cash));
    $(".des-txt").html(data.unit);
    if(data.start_at){
